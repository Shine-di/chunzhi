...
type UniqueIdInfo struct {
	BizType   UniqueBizType `json:"biz_type"`
...
func GenerateUniqueId(bizType UniqueBizType, num int32) ([]int64, error) {
	result := make([]int64, 0)
	var a int32 = 0
	for a = 0; a < num; a++ {
...
func GetUniqueIdInfo(id int64) (*UniqueIdInfo, error) {
	info := new(UniqueIdInfo)
...
func (s *Snowflake) Generate(bizType UniqueBizType) int64 {
	s.Lock()
	defer s.Unlock()
...
func getBizType(id int64) UniqueBizType {
	bizType := (id >> typeShift) & 0x7F
	return UniqueBizType(int32(bizType))
}
